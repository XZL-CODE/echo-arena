...
- **规则单元测试**（26 项）：13 个招式各自的效果（没装时不触发）、回响加伤、反射来回乒乓有上限、同种子的战斗可完全复现、每场都在时限内结束且模拟够快；一轮的场次安排、装配规则、奖励生成、失败重试保持同一种子、存档往返与损坏存档处理、结算提示只陈述记录到的事实，以及在轻松难度下用自动策略打通完整一轮。
...
- **声音没有实际听过**（开发环境没有音频输出），只确认了不报错、可以静音和调音量。
