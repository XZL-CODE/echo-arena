...
- **客户端端到端测试**（9 项，用 Playwright 直接驱动客户端）：启动 → 开始 → 战斗并写入存档；打赢 → 挑奖励 → 进入第 2 场；战斗中关闭再打开 → 回到该场战前准备；输了原样再来 / 回去调整；换开局招式、空格暂停、失焦自动暂停、按 1 再点场地发动冲锋；卸下 / 装回招式、拖动队员；打完最后一场 → 整轮结算 → 再来一轮；设置重启后保留；清除存档。页面只加载本地资源，会联网下载词典的内置拼写检查已关闭。
